...
int main()
{
  ...
  int s_set[6] = {-97364, -71561, -69336, 19675, 71561, 97863};

  for (int i = 0; i < 6; i++)
    {
      if ((std::abs(s_set[i]) == s_set[5 - i] && s_set[i] != s_set[5 - i]) || s_set[i] == 0)
        {
          std::cout << 1 << std::endl;
          break;
        }
  ...
  /* Subset Sum Bonus WIP*/
  int bonus_set[10] = {-98634, -86888, -48841, -40483, 2612, 9225, 17848, 71967, 84319, 88875};
  int middle_neg_indx, middle_pos_indx;
  for (int i = 0; i < 10; i++)
    {
      if (bonus_set[i] < 0)
        {
  ...
  std::vector<int> full_set(bonus_set, bonus_set + 10);
  for (int n = 0; n < middle_neg_indx; n++)
    {
      int temp_neg_sum = 0;
      do
        {
          temp_neg_sum = std::accumulate(bonus_set + n, bonus_set + middle_neg_indx + 1, 0);
        } while (std::next_permutation(bonus_set + n, bonus_set + middle_neg_indx));

      full_set.push_back(temp_neg_sum);
    }

  for (int n = middle_pos_indx + 1; n < 10; n++)
    {
      int temp_pos_sum = 0;
      do
        {
          temp_pos_sum = std::accumulate(bonus_set + middle_pos_indx, bonus_set + n + 1, 0);
        } while (std::next_permutation(bonus_set + middle_pos_indx, bonus_set + n));

      full_set.push_back(temp_pos_sum);
    }
  ...
  for (int i = 0; i < full_set.size(); i++)
    {
      if ((std::abs(full_set[i]) == full_set[full_set.size() - i] && full_set[i] != full_set[full_set.size() - i]) || full_set[i] == 0)
        {
          std::cout << 1 << std::endl;
          break;
        }
